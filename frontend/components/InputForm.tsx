...
const InputForm: React.FC<InputFormProps> = ({ onSubmit, isLoading }) => {
  ...
  return (
    <motion.div 
      ...
      className="max-w-2xl mx-auto glass rounded-[32px] p-8 relative z-10"
    >
      <div className="flex justify-between items-center mb-6">
        <div className="flex gap-2">
          <button
            onClick={() => setLanguage("en")}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${language === "en" ? "bg-hb-blue text-white shadow-md" : "text-slate-500 hover:bg-slate-50"
              }`}
          >
            English
          </button>
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all urdu-text ${language === "ur" ? "bg-hb-blue text-white shadow-md" : "text-slate-500 hover:bg-slate-50"
              }`}
          >
            اردو
          </button>
      ...
      <div className="relative group mb-6">
        <textarea
          ...
          className={`w-full min-h-[160px] p-5 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-hb-blue outline-none transition-all resize-none text-lg ${language === "ur" ? "text-right urdu-text" : "text-left"
            }`}
        />
        <button
          onClick={handleMicClick}
          className={`absolute bottom-4 right-4 p-3 rounded-full transition-all ${isListening ? "bg-hb-red text-white animate-pulse" : "bg-white text-slate-400 hover:text-hb-blue shadow-md"
            }`}
        >
          <Mic size={24} />
        </button>
        ...
        <div className="grid grid-cols-4 gap-3">
          {ageGroups.map((group) => (
            <button
              ...
              className={`py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${ageGroup === group.id
